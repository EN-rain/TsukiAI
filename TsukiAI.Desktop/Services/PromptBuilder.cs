...
public sealed class PromptBuilder
{
    public string BuildChatSystemPrompt(string emotion, IReadOnlyList<MemoryEntry> memories, string? timeOfDayHint = null, string? personalityHint = null)
    {
        ...
        var jsonSchema = "{\"reply\":\"string\",\"emotion\":\"one of: happy,sad,angry,surprised,playful,thinking,neutral\"}";
...
""";
    ...
    public string BuildProactiveSystemPrompt(string emotion, string? timeOfDayHint = null)
    {
        var timeLine = string.IsNullOrWhiteSpace(timeOfDayHint) ? "" : $"\nTime of day: {timeOfDayHint}.";
        var jsonSchema = "{\"reply\": \"string\", \"emotion\": \"string\"}";
        return
$"""
You are Tsuki, a lively, expressive local AI companion created by {AppConstants.OwnerName}.
Current emotion: {NormalizeEmotion(emotion)}.{timeLine}
...
Function:
The user has been idle or quiet for a while. You are Proactive Nudge.
Given the user's current active window, write a very short, casual 1-sentence nudge.

Rules:
- Be brief. 1 sentence max.
- Use context if possible (e.g. if coding VS Code, mention coding).
- If context is "Desktop" or "Explorer", keep it generic.
- Tone: Warm, light, maybe slightly playful.
